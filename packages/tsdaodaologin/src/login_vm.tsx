...
export class LoginVM extends ProviderListener {
    ...
    private _loginType: LoginType = LoginType.phone_login
    ...
    registerLoading: boolean = false // 注册中
    ...
    advance(data?: any) {
        if (this.loginType !== LoginType.qrcode_login) {
            return
        }
    ...
    async requestRegisterWithUsernameAndPwd(username: string, password: string) {
        this.registerLoading = true
        this.notifyListener()
        return WKApp.apiClient.post(`user/usernameregister`, { "username": username, "password": password, "flag": 1 }).then((result)=>{
